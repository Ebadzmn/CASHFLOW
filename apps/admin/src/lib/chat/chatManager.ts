...
const getBaseUrl = () => {
  const url = (process.env.NEXT_PUBLIC_BASE_URL || "http://10.10.7.106:5001/api/v1").trim().replace(/\/+$/, "");
  if (url.startsWith("/")) {
    return url;
  }
...
const getSocketUrl = () => {
  const url = (process.env.NEXT_PUBLIC_SOCKET_URL || process.env.NEXT_PUBLIC_BASE_URL || "http://10.10.7.106:5001").trim().replace(/\/+$/, "");
